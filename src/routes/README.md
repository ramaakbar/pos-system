...
| `/(main)`            | -    | `Main`         | `<Main.Link>`         |
| `/api/[...endpoint]` | -    | `ApiEndpoint`  | `<ApiEndpoint.Link>`  |
...
- [ ] `/(main)/page.info.ts`: Add search typing to if the page supports search paramaters
- [ ] Convert `Link` components for `/(main)` to `<Main.Link>`
- [ ] `/api/[...endpoint]/route.info.ts`: Add search typing to if the page supports search paramaters
- [ ] Convert `Link` components for `/api/[...endpoint]` to `<ApiEndpoint.Link>`
- [ ] Convert `params` typing in `/api/[...endpoint]/route.ts` to `z.infer<>`
